...
### Let us fit a GLM to our data. This will be our global model. use "LossesIndicator"
#as the dependent variable, while the other variables will be predictor variables.
freqfitt <- glm(LossesIndicator ~ UpdatedDay + UpdatedTime +
                 TradedDay + TradedTime + Desk + CapturedBy +
                 TradeStatus + TraderId + Instrument + Reason
               + EventTypeCategoryLevel1 + BusinessLineLevel1,
data=crs$testing, family=poisson(link = 'log'), offset = log(Exposure))
...
cl <- makeCluster(4) # Assign R cores to the job
options(na.action=na.fail)
freqfitts <- dredge(freqfitt)
freqfitts
stopCluster(cl)

cl <- makeCluster(4) # Assign R cores to the job
 atmodel <- (model.avg(get.models(freqfitts, subset=delta<2)))
 summary(atmodel)
 stopCluster(cl)
 
av.predt <- predict(atmodel, crs$training, type = "response")
av.predt

library(R2HTML)
HTMLStart(); HTML(data.frame(av.predt)); w <- HTMLStop()
browseURL(w)

MASS::fitdistr(av.predt, "Poisson")

Estimate <- "file:///C:/Users/User/Documents/R PROJECT/OpRiskPHDGitHub/OpRisk_PHD_Thesis/Data/OPriskDataSet_Test.csv"
PRED <- read.csv(Estimate,
                  sep=";",
                  dec=",",
                  na.strings=c(".", "NA", "", "?"),
                  strip.white=TRUE, encoding="UTF-8")
head(PRED)
library(e1071)
confusionMatrix(table(PRED$Response_test, crs[["testing"]][["LossesIndicator"]]))
